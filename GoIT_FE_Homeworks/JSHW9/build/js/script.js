...
$(function () {
  ...
  skills = _.sortBy(skills, function (i) {
    return i.toLowerCase();
  });
  console.log('skills: ', skills);

  var names = _.map(data, function (obj) {
    return {
      name: obj.name,
      friends: obj.friends.length
    };
  });
  var names = _.orderBy(names, ['friends', 'name'], ['asc, asc']);
  var names = _.map(names, 'name');
  console.log('names: ', names);

  var friends = _.map(data, 'friends');
  friends = _.flattenDeep(friends);
  friends = _.map(friends, 'name');
  friends = _.uniq(friends);
  friends = _.sortBy(friends, function (i) {
    return i.toLowerCase();
  })
